	...
	if($update=='nebeng'){ //kode untuk request nebeng
		...
		if ($values[0] == NULL){ //jika id tebengan yang dituju tidak terdapat di tabel
			...
			if ($values[0] == '0'){ //jika kapasitas nya nol
				...
				$query_cek1		= mysql_query("SELECT user_id FROM nebeng_beri_tebengan WHERE user_id = '$id_penebeng'") or die(mysql_error());		//cek apakah si penebeng merupakan pemberi tebengan atau tidak
				$query_cek2		= mysql_query("SELECT id_penebeng FROM nebeng_nebeng WHERE id_penebeng = '$id_penebeng'") or die(mysql_error());	//cek apakah si penebeng sudah menebeng sebelumnya
